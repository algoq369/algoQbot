...
PORTFOLIO=$(tail -100 "$LOGFILE" 2>/dev/null | grep "Portfolio value updated" | tail -1 | sed 's/.*→ //' | sed 's/,.*//')
DISTRIBUTION=$(tail -100 "$LOGFILE" 2>/dev/null | grep "PORTFOLIO CHECK DEBUG.*Percentages" | tail -1 | sed 's/.*Percentages: //')
...
PRICE=$(tail -100 "$LOGFILE" 2>/dev/null | grep "Current Price:" | tail -1 | awk '{print $NF}')
VOL_4H=$(tail -100 "$LOGFILE" 2>/dev/null | grep "Volatility.*4h" | tail -1 | sed 's/.*4h Volatility: //' | awk '{print $1}')
REGIME=$(tail -100 "$LOGFILE" 2>/dev/null | grep "REGIME.*Detected:" | tail -1 | awk '{print $NF}' | tr -d '",')
...
LAST_TRADES=$(tail -500 "$LOGFILE" 2>/dev/null | grep "Shadow Trade:" | tail -3)
if [ ! -z "$LAST_TRADES" ]; then
    echo "$LAST_TRADES" | while read line; do
        ACTION=$(echo "$line" | grep -o 'buy\|sell' | head -1)
        case "$ACTION" in
            "buy")
                echo -e "  ${GREEN}$line${NC}" | sed 's/.*Shadow Trade: //'
                ;;
            "sell")
                echo -e "  ${RED}$line${NC}" | sed 's/.*Shadow Trade: //'
                ;;
            *)
                echo -e "  ${BLUE}$line${NC}" | sed 's/.*Shadow Trade: //'
                ;;
        esac
    done
else
...
ACTIVE_POS=$(tail -100 "$LOGFILE" 2>/dev/null | grep "activePositions size:" | tail -1 | awk '{print $NF}')
if [ ! -z "$ACTIVE_POS" ]; then
    echo -e "  Active: ${BLUE}$ACTIVE_POS${NC}"
else
